...
!function(){
	var 
		...
		formUrl = {
			// 面授
			ms : 'http://px.thea.cn/Listen/ListenAddAll'
		},
		...
		m = {
		    ...
		    getCity : function(par,c){
		        ...
		        var $par = par;
		        // 不接收默认城市
		        if ($('.user-city',$par).size()>=1 && $('.user-city',$par).hasClass('custom-city') ) return;   //q1
		        // 没有默认城市
		        if (defaultsC == 0 && $('.user-city',$par).size()>=1) {
		            $.ajax({
		                url:'http://proxytheapx.thea.cn/Px/City/ajaxGetProxyCity',
		                type:'get',
		                dataType:'jsonp',
		                jsonp: "callback",
		                jsonpCallback:'call'+(new Date()).getTime(), //加上时间参数不会缓存
		            ...
		            if ($('.user-city',$par).size()>=1) {
		                $.ajax({
		                    url:'http://proxytheapx.thea.cn/Px/City/ajaxGetCityInfo/id/'+defaultsC,
		                    type:'get',
		                    dataType:'jsonp',
		                    jsonp: "callback",
		    ...
		    getArea : function(par,c){
		        if ($('.user-area',par).size()>=1 && c != 0) {
		            // 请求地区；
		            $.ajax({
		                url:'http://proxy21px.thea.cn/Px/City/ajaxGetArea/id/'+ c,
		                type:'get',
		                dataType:'jsonp',
		                jsonp: "callback",
                		jsonpCallback:'call'+(new Date()).getTime(),
		    ...
		    selectCity : function (par){
				...
				console.log('selectcity GO');
				console.log(par);
				
		    	par.each(function(){
		    		var $this = $(this); //优化性能 存储this
		    		$('.user-city',$this).change(function(){
			    		var defaultsC = $('option:selected',$this).val();//改变的城市的值
